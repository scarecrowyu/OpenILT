...
class FragmentsOneEdge:
    def __init__(self, direction = None, ref_edge = None): 
        ...
        self._epe_edge_pairs = None
        self._epe_edge_offsets = None
        self._direction = direction
        self._ref_edge = ref_edge
        self._epe_checkpoints = None
    
    def add_corner_edge_pairs(self, edge_pairs):
        self._corner_edge_pairs = edge_pairs.long()
        self._corner_offsets = torch.zeros(edge_pairs.shape[0],device = DEVICE).long()
    
    def add_projection_edge_pairs(self, edge_pairs):
        self._projection_edge_pairs = edge_pairs.long()
        self._projection_offsets = torch.zeros(edge_pairs.shape[0],device = DEVICE).long()

    def add_normal_edge_pairs(self, edge_pairs):
        self._normal_edge_pairs = edge_pairs.long()
        self._normal_offsets = torch.zeros(edge_pairs.shape[0],device = DEVICE).long()
...
class Mask:
    ...
    def find_contour(self):
        np_mask = self._target.clone().detach().cpu().numpy()
        ...
        for contour in contours: 
            ...
            self._polygons.append(torch.tensor(contour_switch_xy).to(self._device))
            ...
            for i in range(1, len(polygon)):
                ...
                if edge[i-1][1]==0: # Vertical
                    if p2[0] > p1[0]: # means the right side of edge is inside the target, downward edge
                        forbidden_region = self._mask[p1[0]-range1: p2[0]+range1, p1[1]-range1 : p1[1]]
                        # forbidden_region = target[p1[0]: p2[0], p1[1]-range1 : p1[1]]  
                        if not(forbidden_region.any().item()):
                            # sraf1_region = target[p1[0]-range1-range2 : p2[0]+range1+range2, p1[1]-range1-range2 : p1[1]] 
                            sraf1_region = self._mask[p1[0] : p2[0], p1[1]-range1-range2 : p1[1]]
                            if sraf1_region.any().item():
                                proj_edge_col_index = torch.nonzero(sraf1_region.any(dim=0))[-1]
                                srafy,srafx1 = (p1[0]+p2[0])//2, (p1[1] +p1[1]+ proj_edge_col_index-range1-range2)//2
                        ...
                        forbidden_region = self._mask[p2[0]-range1 : p1[0]+range1 , p1[1]+1 : p1[1]+range1+1]
                        # forbidden_region = target[p2[0] : p1[0] , p1[1]+1 : p1[1]+range1+1]
                        if not(forbidden_region.any().item()):
                            # sraf1_region = target[p2[0]-range1-range2 : p1[0]+range1+range2, p1[1]+1 : p1[1]+range1+range2+1]
                            sraf1_region = self._mask[p2[0] : p1[0], p1[1]+1 : p1[1]+range1+range2+1] 
                            if sraf1_region.any().item():
                                proj_edge_col_index = torch.nonzero(sraf1_region.any(dim=0))[0]
                                srafy,srafx1 = (p1[0]+p2[0])//2, (p1[1] +p1[1]+ proj_edge_col_index)//2
                    ...
                    if p2[1]>p1[1]: # means the upper side of edge is inside the target, rightward edge
                        forbidden_region = self._mask[p1[0]+1: p1[0]+range1+1, p1[1]-range1: p2[1]+range1] 
                        # forbidden_region = target[p1[0]+1: p1[0]+range1+1, p1[1]: p2[1]] 
                        if not(forbidden_region.any().item()): 
                            # sraf1_region = target[p1[0]+1: p1[0]+range1+range2+1, p1[1]-range1-range2 : p2[1]+range1+range2]
                            sraf1_region = self._mask[p1[0]+1: p1[0]+range1+range2+1, p1[1] : p2[1]]  
                            if sraf1_region.any().item():        
                                proj_edge_row_index = torch.nonzero(sraf1_region.any(dim=1))[0]     
                                srafy1, srafx = (p1[0]+p1[0]+proj_edge_row_index)//2,  (p1[1]+p2[1])//2
                        ...
                        forbidden_region = self._mask[p1[0]-range1: p1[0], p2[1]-range1: p1[1]+range1]
                        # forbidden_region = target[p1[0]-range1: p1[0], p2[1]: p1[1]]
                        if not(forbidden_region.any().item()): 
                            # sraf1_region = target[p1[0]-range1-range2: p1[0], p2[1]-range1-range2 : p1[1]+range1+range2]
                            sraf1_region = self._mask[p1[0]-range1-range2: p1[0], p2[1] : p1[1]]
                            if sraf1_region.any().item():
                                proj_edge_row_index = torch.nonzero(sraf1_region.any(dim=1))[-1]
                                srafy1, srafx = (p1[0]+p1[0]-range1-range2+proj_edge_row_index)//2,  (p1[1]+p2[1])//2
            ...
            for i in range(1, len(polygon)):   
                ...
                if edge[i-1][1] == 0:  # vertical 
                    ...
                    if target_slice.any().item():
                        ...
                        padded_col = torch.flatten(torch.cat((torch.zeros((1, 1), device = DEVICE), proj_edge_col, torch.zeros((1, 1), device = DEVICE)),dim=0))
                    ...
                    if(abs(edge[i-1][0])<lengthMin):
                        ...
                        start_points = torch.stack([p1+ i*torch.tensor([lengthNormal, 0],device = DEVICE) for i in range(unifrag_count+1)]) if p1[0] < p2[0] else torch.stack([p1 - i*torch.tensor([lengthNormal, 0], device = DEVICE) for i in range(unifrag_count+1)])
                        ...
                        corner_start1 = torch.cat((p1, torch.tensor([p1[0]+lengthCorner-1, p1[1]],device = DEVICE))) if p1[0] < p2[0] else torch.cat((p1, torch.tensor([p1[0]-lengthCorner+1, p1[1]],device = DEVICE))) #torch.size([4])
                        corner_end1 = torch.cat((torch.tensor([p2[0] - lengthCorner + 1, p2[1]],device = DEVICE), p2)) if p1[0] < p2[0] else torch.cat((torch.tensor([p2[0]+lengthCorner -1, p2[1]],device = DEVICE), p2)) 
                        ...
                        start_points = torch.stack([torch.tensor([p1[0]+ lengthCorner, p1[1]],device = DEVICE) + i*torch.tensor([lengthNormal, 0],device = DEVICE) for i in range(unifrag_count + 1)]) if p1[0] < p2[0] else torch.stack([torch.tensor([p1[0] - lengthCorner, p1[1]],device = DEVICE) - i*torch.tensor([lengthNormal, 0],device = DEVICE) for i in range(unifrag_count+1)])
                        end_points = start_points[1:,:].clone()
                        end_points[:,0] = end_points[:,0] - 1 if p1[0] < p2[0] else end_points[:,0] + 1
                        end_points = torch.cat((end_points, torch.tensor([p2[0] - lengthCorner, p2[1]],device = DEVICE).unsqueeze(0)), dim=0) if p1[0]<p2[0] else torch.cat((end_points, torch.tensor([p2[0] + lengthCorner, p2[1]],device = DEVICE).unsqueeze(0)), dim=0)
                    ...
                    if target_slice.any().item():
                        ...
                        padded_row = torch.flatten(torch.cat((torch.zeros((1, 1), device = DEVICE), proj_edge_row, torch.zeros((1, 1), device = DEVICE)),dim=1))
                        start_indices = torch.where(torch.diff(padded_row)==1)[0]
                        end_indices = torch.where(torch.diff(padded_row)==-1)[0]-1
                        edge_pairs = torch.stack((torch.full_like(start_indices, p1[0]), min(p1[1], p2[1])+start_indices, torch.full_like(start_indices, p1[0]), min(p1[1], p2[1])+end_indices),dim=1) #[y, x1, y, x2]
                        fragment.add_projection_edge_pairs(edge_pairs)# Add projection fragments corresponding to one target edge
                    # Corner segments and uniform segments
                    if(abs(edge[i-1][1])<lengthMin):
                        unifrag_count = (abs(edge[i-1][1])) // lengthNormal
                        start_points = torch.stack([p1+ i*torch.tensor([0, lengthNormal],device = DEVICE) for i in range(unifrag_count+1)]) if p1[1] < p2[1] else torch.stack([p1 - i*torch.tensor([0, lengthNormal],device = DEVICE) for i in range(unifrag_count+1)])
                        end_points = start_points[1:, :].clone()
                        ...
                        # First consider corner small fragments 
                        corner_start1 = torch.cat((p1, torch.tensor([p1[0], p1[1]+lengthCorner-1],device = DEVICE))) if p1[1] < p2[1] else torch.cat((p1, torch.tensor([p1[0], p1[1]-lengthCorner+1],device = DEVICE)))
                        corner_end1 = torch.cat((torch.tensor([p2[0], p2[1]-lengthCorner + 1],device = DEVICE), p2)) if p1[1] < p2[1] else torch.cat((torch.tensor([p2[0], p2[1]+lengthCorner-1],device = DEVICE), p2))
                        ...
                        start_points = torch.stack([torch.tensor([p1[0], p1[1]+ lengthCorner],device = DEVICE)+ i* torch.tensor([0, lengthNormal],device = DEVICE) for i in range(unifrag_count+1)]) if p1[1] < p2[1] else torch.stack([torch.tensor([p1[0], p1[1]- lengthCorner],device = DEVICE) - i*torch.tensor([0, lengthNormal],device = DEVICE) for i in range(unifrag_count+1)])
                        end_points = start_points[1:,:].clone()
                        end_points[:,1] = end_points[:,1] - 1 if p1[1] < p2[1] else end_points[:,1] + 1
                        end_points = torch.cat((end_points, torch.tensor([p2[0], p2[1]- lengthCorner],device = DEVICE).unsqueeze(0)), dim=0) if p1[1]<p2[1] else torch.cat((end_points, torch.tensor([p2[0], p2[1]+lengthCorner],device = DEVICE).unsqueeze(0)), dim=0)
                        edge_pairs = torch.cat((start_points, end_points),dim=1)
                        if edge_pairs[-1][1] == edge_pairs[-1][3]:
                            edge_pairs[-2][3] = edge_pairs[-1][3]
                            edge_pairs = edge_pairs.narrow(0, 0, edge_pairs.shape[0] - 1)
    ...
    def update_fragments(self, projection_step, corner_step, normal_step, nominalImage, epeprobe):
        """
        ...
        for edge_h in self._fragments_h:
            if edge_h.direction() == 'right':
                ...
                image_outer = nominalImage[ref_points[:,0]+epeprobe, ref_points[:,1]]
                image_inner = nominalImage[ref_points[:,0]-epeprobe, ref_points[:,1]]
                ...
                image_outer = nominalImage[ref_points[:,0]-epeprobe, ref_points[:,1]]
                image_inner = nominalImage[ref_points[:,0]+epeprobe, ref_points[:,1]]
                edge_h._normal_offsets[image_outer >= 0.5] -= normal_step # image expand, mask should shrink
                edge_h._normal_offsets[image_inner <= 0.5] += normal_step # image shrink, mask should expand
                #Corner edge
                if edge_h._corner_edge_pairs != None:
                    corner_pairs = edge_h._corner_edge_pairs
                    ref_points = torch.stack((corner_pairs[:,0],(corner_pairs[:,1]+corner_pairs[:,3])//2), dim=1)
        ...
        for edge_v in self._fragments_v:
            if edge_v.direction() == 'down': #rightside is inside target
                #Normal edge 
                normal_pairs = edge_v._normal_edge_pairs
                ref_points = torch.stack(((normal_pairs[:,0]+normal_pairs[:,2])//2, normal_pairs[:,1]), dim=1)
                image_outer = nominalImage[ref_points[:,0], ref_points[:,1]-epeprobe]
                image_inner = nominalImage[ref_points[:,0], ref_points[:,1]+epeprobe]
                ...
                if edge_h._projection_edge_pairs != None:
                    ...
                    for i in range(projection_offsets.shape[0]):
                        ...
                        if projection_offsets[i] > 0.01:
                            mask[projection_pairs[i,0]-innermost+1 : projection_pairs[i,0]+1, projection_pairs[i,1] : projection_pairs[i,3]+1] = 1
            ...
            if edge_v.direction() == 'up':
                ...
                for i in range(normal_offsets.shape[0]):
                    if normal_offsets[i] > 0.01:
                        ...
                        mask[normal_pairs[i,2] : normal_pairs[i,0]+1, normal_pairs[i,1]+normal_offsets[i]+1 : normal_pairs[i,1]+1] = 0
                if edge_v._corner_edge_pairs != None:
                    corner_pairs = edge_v._corner_edge_pairs
                    corner_offsets = edge_v._corner_offsets
                    assert corner_pairs.shape[0] == corner_offsets.shape[0]
                    for i in range(corner_offsets.shape[0]): 
                        if corner_offsets[i] > 0.01:
                            ...
                            mask[corner_pairs[i,2] : corner_pairs[i,0]+1, corner_pairs[i,1]+corner_offsets[i]+1 : corner_pairs[i,1]+1] = 0                
                if edge_v._projection_edge_pairs != None:
                    projection_pairs = edge_v._projection_edge_pairs
                    projection_offsets = edge_v._projection_offsets
                    outermost=torch.max(torch.max(corner_offsets), torch.max(normal_offsets))
                    ...
                    for i in range(projection_offsets.shape[0]):
                        ...
                        mask[projection_pairs[i,2] : projection_pairs[i,0]+1, projection_pairs[i,1]-innermost+1 : projection_pairs[i,1]+1] = 1
                        if projection_offsets[i] > 0.01:
                            mask[projection_pairs[i,2] : projection_pairs[i,0]+1, projection_pairs[i,1]+1 : projection_pairs[i,1]+1+projection_offsets[i]] = 1
                        elif projection_offsets[i] <= -0.01:
                            mask[projection_pairs[i,2] : projection_pairs[i,0]+1, projection_pairs[i,1]+projection_offsets[i]+1 : projection_pairs[i,1]+1] = 0
        return mask
    
    # def updateMask_EPE(self, epep, projection_step, corner_step, normal_step, nominalImage):
    #     # new_frag_h = sorted(self._fragments_h, key=lambda x : x._ref_edge[0].item(), reverse=False)
    #     # new_frag_v = sorted(self._fragments_v, key=lambda x : x._ref_edge[1].item(), reverse=False)
    #     for h in self._fragments_h:
    #         if h._corner_offsets == None:
    #             cat_edge_pairs = h._normal_offsets
    #         else:
    #             cat_edge_pairs = torch.cat([self._corner_edge_pairs[0].unsqueeze(0), self._normal_edge_pairs, self._corner_edge_pairs[1].unsqueeze(0)])
    #         for i in range(1, len(cat_edge_pairs)):
    #             if epep
