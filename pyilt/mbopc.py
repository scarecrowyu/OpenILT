...
class MbOPC:
    ...
    def solve(self, target, pattern:Mask, curv = None, verbose = 0):
        ...
        bestParams = None
        bestMask = None
        pattern.fragment_edge(projection=30, lengthCorner=5, lengthNormal=10, lengthMin=50)
        ...
        for idx in range(cfg._config["Iterations"]): 
            ...
            if idx <10:
                pattern.update_fragments(projection_step=2, corner_step=2, normal_step=3, nominalImage=printedNom)
            else:
...
design = glp.Design(f"./benchmark/ICCAD2013/M1_test1.glp", down=SCALE)
...
pattern.fragment_edge(projection=30, lengthCorner=40, lengthNormal=20, lengthMin=81)
# pattern.fragment_edge(projection=30, lengthCorner=5, lengthNormal=10, lengthMin=50)
pattern.add_sraf(range1=100, range2=50, distance2=120, distance3=150, width1=20, width2=20, width3=10, lratio1=0.8, lratio2=0.6, lratio3=0.4)
# pattern.add_sraf(range1=30, range2=30, distance2=100, distance3=150, width1=20, width2=20, width3=10, lratio1=0.8, lratio2=0.6, lratio3=0.4)
mask = pattern._mask
for idx in range(cfg._config["Iterations"]): 
    printedNom, printedMax, printedMin = solver._lithosim(mask)
